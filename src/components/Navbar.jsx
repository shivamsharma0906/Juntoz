      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        `}>
          ...
          <a href="#" className="flex items-center gap-2 group">
            <img src={logo} alt="Juntoz Logo" className="h-10 md:h-12 w-auto group-hover:md:scale-110 transition-transform" />
          </a>
          ...
          <div className="hidden md:flex items-center gap-2 bg-muted rounded-full p-1 border-2 border-dashed border-accent-5">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className={`font-heading font-bold uppercase tracking-widest text-sm text-white px-6 py-2 rounded-full md:transition-all md:duration-300 ${link.color}`}
              >
                {link.name}
              </a>
            ))}
